---
title: "RNA-Seq and Expression Arrays: Selection Guidelines for Genome-wide Expression Profiling"
author: "Jessica Minnier, OHSU"
date: "July 28, 2017"
output:
  html_document:
    code_folding: hide
...
```{r figure2, dpi=400, fig.width=4.5,fig.height=4,fig.ext="tiff"}
...
tmpdat = results_all%>%arrange(desc(abs(logFC_RNAseq_limma)))%>%
  select(ensembl_id,gene_name,gene_biotype,`locus type`,logFC_RNAseq_limma,logFC_MA_limma,
...
results_all_full%>%filter(qvalue_MA_Storey<.05)%>%rownames_to_column("id")%>%
  ...
  select(id,logFC_RNAseq_limma,logFC_MA_limma,
